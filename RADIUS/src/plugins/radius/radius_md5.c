...
static void
radius_md5_transform(radius_md5_ctx_t *ctx, const u8 data[64])
{
  u32 a, b, c, d, m[16], i, j;

  for (i = 0, j = 0; i < 16; ++i, j += 4)
    m[i] = (u32)data[j] | ((u32)data[j + 1] << 8) |
           ((u32)data[j + 2] << 16) | ((u32)data[j + 3] << 24);
...
void radius_md5_update(radius_md5_ctx_t *ctx, const u8 *data, u32 len)
{
  ...
  for (i = 0; i < len; ++i) {
    ...
    ctx->datalen++;
    if (ctx->datalen == 64) {
      radius_md5_transform(ctx, ctx->data);
      ctx->bitlen += 512;
      ctx->datalen = 0;
    }
  }
...
void radius_md5_final(radius_md5_ctx_t *ctx, u8 hash[16])
{
  ...
  for (i = 0; i < 4; ++i) {
    hash[i]      = (u8)((ctx->state[0] >> (8 * i)) & 0xff);
    hash[i + 4]  = (u8)((ctx->state[1] >> (8 * i)) & 0xff);
    hash[i + 8]  = (u8)((ctx->state[2] >> (8 * i)) & 0xff);
    hash[i + 12] = (u8)((ctx->state[3] >> (8 * i)) & 0xff);
  }
}
