...
static void vl_api_tlsctrl_vpn_pool_set_t_handler (vl_api_tlsctrl_vpn_pool_set_t *mp);
static void vl_api_tlsctrl_vpn_profile_set_t_handler (vl_api_tlsctrl_vpn_profile_set_t *mp);
...
static void
vl_api_tlsctrl_vpn_pool_set_t_handler (vl_api_tlsctrl_vpn_pool_set_t *mp)
{
  vl_api_tlsctrl_vpn_pool_set_reply_t *rmp;
  u8 *name = dup_api_str_fixed (mp->name, sizeof (mp->name));
  u8 *subnet = dup_api_str_fixed (mp->subnet, sizeof (mp->subnet));
  u8 *gateway = dup_api_str_fixed (mp->gateway, sizeof (mp->gateway));
  int rv = tlsctrl_vpn_pool_set ((char *) name, (char *) subnet,
                                 (char *) gateway,
                                 clib_net_to_host_u32 (mp->lease_seconds));
  REPLY_MACRO (VL_API_TLSCTRL_VPN_POOL_SET_REPLY);
  vec_free (name);
  vec_free (subnet);
  vec_free (gateway);
}
...
static void
vl_api_tlsctrl_vpn_profile_set_t_handler (vl_api_tlsctrl_vpn_profile_set_t *mp)
{
  vl_api_tlsctrl_vpn_profile_set_reply_t *rmp;
  u8 *name = dup_api_str_fixed (mp->name, sizeof (mp->name));
  u8 *pool = dup_api_str_fixed (mp->pool, sizeof (mp->pool));
  u8 *dns = dup_api_str_fixed (mp->dns_servers, sizeof (mp->dns_servers));
  u8 *inc = dup_api_str_fixed (mp->include_routes, sizeof (mp->include_routes));
  u8 *exc = dup_api_str_fixed (mp->exclude_routes, sizeof (mp->exclude_routes));
  int rv = tlsctrl_vpn_profile_set ((char *) name, (char *) pool,
                                    mp->full_tunnel,
                                    (char *) dns, (char *) inc, (char *) exc,
                                    clib_net_to_host_u16 (mp->mtu),
                                    clib_net_to_host_u16 (mp->mss_clamp));
  REPLY_MACRO (VL_API_TLSCTRL_VPN_PROFILE_SET_REPLY);
  vec_free (name);
  vec_free (pool);
  vec_free (dns);
  vec_free (inc);
