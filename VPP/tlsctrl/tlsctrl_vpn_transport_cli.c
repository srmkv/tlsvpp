...
static clib_error_t *
set_tlsctrl_vpn_transport_drain_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  u32 tunnel_id = 0, budget = 0;
  tlsctrl_vpn_transport_session_t *s = 0;

  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "tunnel-id %u", &tunnel_id)) ;
      else if (unformat (input, "budget %u", &budget)) ;
      else break;
    }
  ...
  if (tlsctrl_vpn_transport_find_session (tunnel_id, &s))
    return clib_error_return (0, "transport session not found");

  if (budget >= s->queue_depth)
    s->queue_depth = 0;
  else
    s->queue_depth -= budget;

  return 0;
}

VLIB_CLI_COMMAND (set_tlsctrl_vpn_transport_drain_cmd, static) = {
  .path = "tlsctrl vpn transport drain",
  .short_help = "tlsctrl vpn transport drain tunnel-id <id> budget <n>",
  .function = set_tlsctrl_vpn_transport_drain_fn,
};

static clib_error_t *
set_tlsctrl_vpn_transport_reset_error_fn (vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
  u32 tunnel_id = 0;
  tlsctrl_vpn_transport_session_t *s = 0;

  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    if (unformat (input, "tunnel-id %u", &tunnel_id)) ; else break;
...
VLIB_CLI_COMMAND (set_tlsctrl_vpn_transport_reset_error_cmd, static) = {
  .path = "tlsctrl vpn transport reset-error",
  .short_help = "tlsctrl vpn transport reset-error tunnel-id <id>",
  .function = set_tlsctrl_vpn_transport_reset_error_fn,
};
