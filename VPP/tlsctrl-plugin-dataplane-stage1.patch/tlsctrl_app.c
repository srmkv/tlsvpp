...
static u64
tlsctrl_json_extract_u64 (u8 *json, const char *key, u64 fallback)
{
  u8 *v = tlsctrl_json_extract_value (json, key);
  u64 out = fallback;
  if (v && vec_len (v))
    out = strtoull ((char *) v, 0, 10);
  vec_free (v);
  return out;
}

static int
tlsctrl_hex_nibble (u8 c)
{
  if (c >= '0' && c <= '9')
    return c - '0';
  if (c >= 'a' && c <= 'f')
...
static u8 *
tlsctrl_hex_encode_dup (const u8 *data, u32 len)
{
  static const char hex[] = "0123456789abcdef";
  u8 *out = 0;
  u32 i;
  if (!data || !len)
    return tlsctrl_dup_c_string ((const u8 *) "", 0);
  vec_validate (out, (len * 2));
  for (i = 0; i < len; i++)
    {
...
static int
tlsctrl_hex_decode_dup (u8 *hexs, u8 **out)
{
  u32 n, i;
  if (out)
    *out = 0;
  ...
  if (n == 0)
    return 0;
  if (out)
    vec_validate (*out, (n / 2) - 1);
  for (i = 0; i < n; i += 2)
    {
      int hi = tlsctrl_hex_nibble (hexs[i]);
      int lo = tlsctrl_hex_nibble (hexs[i+1]);
      if (hi < 0 || lo < 0)
        {
          if (out)
            { vec_free (*out); *out = 0; }
...
tlsctrl_handle_request (tlsctrl_conn_t *conn)
{
  ...
  if (!strcmp ((char *) method, "POST") &&
           ...
           !strncmp ((char *) target, "/api/client/vpn-bind", 20))
    {
      ...
      tlsctrl_vpn_transport_attach (tunnel_id, "mtls-http");
    ...
    {
      u64 tunnel_id_u64 = tlsctrl_json_extract_u64 (body, "tunnel_id", 0);
      u8 *frame_hex = tlsctrl_json_extract_string (body, "frame_hex");
      u8 *frame = 0, *payload2 = 0;
      tlsctrl_vpn_frame_meta_t meta;
      int rv;
      username = tlsctrl_json_extract_string (body, "username");
      if (!username || !vec_len (username))
        username = tlsctrl_header_value_dup (conn->request_data, header_end, "X-Username:");
      if (tlsctrl_http_authorize_user (username, peer_cert_serial, 1, &auth_status) <= 0)
        {
          if (auth_status == 423)
            tlsctrl_build_json_response (conn, 423, "Locked",
      ...
      if (!tunnel_id_u64 || !frame_hex || tlsctrl_hex_decode_dup (frame_hex, &frame))
        {
          vec_free (frame_hex);
          tlsctrl_build_json_response (conn, 400, "Bad Request",
                                       "{\"ok\":false,\"error\":\"tunnel_id and frame_hex required\"}");
          goto done;
        }
      rv = tlsctrl_vpn_frame_rx (tunnel_id_u64, frame, vec_len (frame), &meta, &payload2);
      if (rv)
        {
          vec_free (frame_hex);
          vec_free (frame);
      ...
      tlsctrl_vpn_transport_attach (tunnel_id_u64, "mtls-http");
      tlsctrl_vpn_transport_enqueue_rx_frame (tunnel_id_u64, frame);
      vec_free (payload2);
      vec_free (frame_hex);
           ...
           !strncmp ((char *) target, "/api/client/vpn-poll", 20))
    {
      u64 tunnel_id_u64 = 0;
      u8 *hex = 0, *frame = 0;
      tunnel_id_u64 = strtoull ((char *) (tlsctrl_query_value_dup (target, "tunnel_id") ?: (u8*)"0"), 0, 10);
      username = tlsctrl_query_value_dup (target, "username");
      if (tlsctrl_http_authorize_user (username, peer_cert_serial, 1, &auth_status) <= 0)
        {
          if (auth_status == 423)
      ...
      if (tlsctrl_vpn_transport_dequeue_tx_frame (tunnel_id_u64, &frame))
        {
          tlsctrl_build_json_response (conn, 200, "OK", "{\"ok\":true,\"frame_hex\":\"\"}");
          goto done;
        }
      hex = tlsctrl_hex_encode_dup (frame, vec_len (frame));
      json = format (0, "{\"ok\":true,\"frame_hex\":\"%s\"}", hex ? (char*)hex : "");
      tlsctrl_build_response (conn, 200, "OK", "application/json", json);
      vec_free (hex);
      vec_free (frame);
    }
  else if (!strcmp ((char *) method, "POST") &&
           !strncmp ((char *) target, "/api/client/vpn-keepalive", 25))
    {
      u64 tunnel_id_u64 = tlsctrl_json_extract_u64 (body, "tunnel_id", 0);
      u8 *frame = 0, *hex = 0;
      username = tlsctrl_json_extract_string (body, "username");
      ...
      if (!tunnel_id_u64 || tlsctrl_vpn_frame_tx_keepalive (tunnel_id_u64, &frame))
        {
          tlsctrl_build_json_response (conn, 400, "Bad Request",
                                       "{\"ok\":false,\"error\":\"keepalive build failed\"}");
          goto done;
        }
      tlsctrl_vpn_transport_enqueue_tx_frame (tunnel_id_u64, frame);
      hex = tlsctrl_hex_encode_dup (frame, vec_len (frame));
      json = format (0, "{\"ok\":true,\"frame_hex\":\"%s\"}", hex ? (char*)hex : "");
      tlsctrl_build_response (conn, 200, "OK", "application/json", json);
      vec_free (hex);
