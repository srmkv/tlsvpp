...
func (u *UI) buildConfigPage() fyne.CanvasObject {
	mainForm := container.New(layout.NewFormLayout(), widget.NewLabel("Адрес сервера"), u.serverURL, widget.NewLabel("Учетная запись"), u.username, widget.NewLabel("Конфигурация"), u.bundleStatus)
	advancedForm := container.New(layout.NewFormLayout(), widget.NewLabel("Имя сервера"), u.serverName, widget.NewLabel("Путь bind"), u.clientsPath, widget.NewLabel("Путь команд"), widget.NewLabel(u.cfg.CommandsPath), widget.NewLabel("Путь отчета приложений"), widget.NewLabel(u.cfg.AppsReportPath), widget.NewLabel("Интервал опроса, сек"), u.pollSeconds)
