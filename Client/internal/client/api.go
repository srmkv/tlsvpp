...
func SendAppsReport(cfg state.Config, report model.AppsReport) error {
	if strings.TrimSpace(cfg.AppsReportPath) == "" {
		return nil
	}
	...
	urlValue := joinURL(cfg.ServerURL, cfg.AppsReportPath)
	req, err := http.NewRequest(http.MethodPost, urlValue, bytes.NewReader(bodyBytes))
	if err != nil {
		return fmt.Errorf("build apps report request: %w", err)
	}
	req.Header.Set("Content-Type", "application/json")
	applyClientHeaders(req)
	resp, err := httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("apps report request failed: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 1024))
		if len(body) > 0 {
			return fmt.Errorf("apps report status: %s: %s", resp.Status, strings.TrimSpace(string(body)))
		}
		return fmt.Errorf("apps report status: %s", resp.Status)
	}
	return nil
}
